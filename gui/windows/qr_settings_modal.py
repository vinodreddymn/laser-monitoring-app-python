...
class QRSettingsModal(ctk.CTkToplevel):
    def __init__(self, parent, model, callback):
        ...
        self.prefix = ctk.StringVar(value=settings.get("qr_text_prefix", "Part"))
        self.counter = ctk.IntVar(value=settings.get("qr_start_counter", 1))

        self.setup_ui()

    def setup_ui(self):
        ctk.CTkLabel(self, text=f"Activate Model: {self.model['name']}", font=ctk.CTkFont(size=20, weight="bold")).pack(pady=20)
        ...
        preview = ctk.CTkLabel(frame, text="", font=ctk.CTkFont(size=18, family="Courier", weight="bold"))
        preview.pack(pady=20)
        def update_preview(*args):
            p = self.prefix.get().strip() or "Text"
            c = str(self.counter.get()).zfill(5)
            preview.configure(text=f"Next QR → {p}-{c}")
        ...
        btns.pack(pady=20)
        ctk.CTkButton(btns, text="Cancel", fg_color="gray", command=self.destroy).pack(side="left", padx=10)
        ctk.CTkButton(btns, text="Activate & Save", fg_color="#10b981", command=self.activate).pack(side="left", padx=10)

    def activate(self):
        save_qr_settings(self.prefix.get().strip(), self.counter.get())
        # ... set active model logic
        self.callback()
        self.destroy()